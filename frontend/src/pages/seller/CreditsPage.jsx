            ...
            {balance && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col justify-center items-start">
                  ...
                  <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">Total Minted</p>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col justify-center items-start">
                  ...
                  <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">Total Sold</p>
                ...
                <div className="bg-[#022c22] border border-[#022c22] rounded-2xl p-6 shadow-md flex flex-col justify-center items-start">
                  ...
                  <p className="text-xs text-[#bef264] font-semibold uppercase tracking-wider mb-1">Remaining Balance</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                <div className="px-6 py-5 border-b border-gray-200 flex items-center gap-2">
                  ...
                  <h2 className="text-base font-semibold text-gray-900 tracking-tight">Issued Batches</h2>
                </div>
                ...
                <div className="flex-1 p-6 bg-gray-50/30">
                  {issued.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="text-gray-500 text-sm">No credit batches have been minted yet.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {issued.map((batch) => (
                        <div key={batch.id} className="bg-white border border-gray-200 rounded-xl p-4 flex justify-between items-center shadow-sm">
                          <div>
                            <span className="block font-semibold text-gray-900 text-sm truncate">{batch.project_title}</span>
                            <span className="text-xs text-gray-500 font-medium">Batch {String(batch.id).substring(0,6).toUpperCase()} • {batch.vintage_year}</span>
                          </div>
                          <div className="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-sm font-bold border border-emerald-100 shrink-0">
                ...
                <div className="flex-1">
                  {history.length === 0 ? (
                    <div className="text-center py-16 bg-gray-50/30 h-full flex flex-col justify-center">
                      ...
                      <p className="text-gray-500 text-sm">No credit transactions yet.</p>
                    </div>
                  ) : (
                      ...
                      {history.map((tx) => (
                        <div key={tx.id} className="px-6 py-4 hover:bg-gray-50 transition-colors flex justify-between items-center">
                          ...
                          <div className="text-right">
                            <span className={`block font-bold text-sm ${tx.transaction_type === 'mint' ? 'text-emerald-600' : 'text-gray-900'}`}>
                              {tx.transaction_type === 'mint' ? '+' : '-'}{tx.amount} credits
                            </span>
                            {tx.price_per_credit && (
                              <span className="text-xs text-gray-500">@ ${tx.price_per_credit}/each</span>
                            )}
                          </div>
                        </div>
                      ))}
