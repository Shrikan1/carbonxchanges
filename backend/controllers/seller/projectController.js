...
const REQUIRED_ON_SUBMIT = [
  ...
  'country', 'total_project_area_hectares', 'project_start_date', 'duration_months',
...
const NUMERIC_CONSTRAINTS = {
  ...
  duration_months: { min: 1, max: 1200 },
  crediting_period_months: { min: 1, max: 1200 },
...
async function replaceKycDocument(req, res) {
  try {
    if (!VALID_SCALES.includes(req.body.project_scale)) {
      return res.status(400).json({
        error: `Invalid project_scale. Must be 'small-scale' or 'large-scale'`,
...
async function replaceKycDocument(req, res) {
  try {
    ...
    if (!['draft', 'pending', 'flagged'].includes(project.status)) {
      return res.status(400).json({ error: `Cannot replace documents while project is ${project.status}` });
    }

    const updatedProject = await Project.updateKycDocPath(project.id, docType, path);
    res.json({ message: 'KYC Document replaced successfully', project: updatedProject });
  } catch (err) {
    console.error('Replace KYC document error:', err);
    res.status(500).json({ error: 'Failed to replace KYC document' });
  }
}
