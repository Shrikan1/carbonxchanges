...
async function findAllUsers({ role, is_seller, is_buyer, limit = 20, offset = 0 } = {}) {
  const conditions = [];
  const filterValues = [];
  let i = 1;

  if (role) { conditions.push(`role = $${i++}`); filterValues.push(role); }
  if (is_seller !== undefined) { conditions.push(`is_seller = $${i++}`); filterValues.push(is_seller); }
  if (is_buyer !== undefined) { conditions.push(`is_buyer = $${i++}`); filterValues.push(is_buyer); }
  ...
  const [dataResult, countResult] = await Promise.all([
    ...
    query(`SELECT COUNT(*) FROM users ${whereClause}`, filterValues),
  ]);
...
async function findAllProjects({ status, limit = 20, offset = 0 } = {}) {
...
async function findAllTransactions({ limit = 20, offset = 0 } = {}) {
  const [dataResult, countResult] = await Promise.all([
    query(
